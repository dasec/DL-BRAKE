...
bool Client::enroll(MinutiaeView ref, int k, bool verbose)
{
    ...
    // Real version
    // Lock the vault
    FuzzyVaultBake vault(mcytWidth, mcytHeight, mcytDpi);   // should be BRAKE, but the underlying thimble fork would need to be updated
...
bool Client::verify(MinutiaeView query, int k, bool verbose)
{
    ...
    FuzzyVaultBake vault(bv);      // should be BRAKE, but the underlying thimble fork would need to be updated
